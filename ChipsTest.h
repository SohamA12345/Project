 ...
 private:
  ...
  void test_chips_default() {
    ...
    if (c.get_chips_seasoning() != "N/A") {
      cout << "default test for chips seasoning failed, result unexpected "
           << endl;
    }
  ...
  void test_chips_case1() {
    ...
    // then compare the values against the expected
    // cut/seasoning/name/price/size
  ...
  void test_chips_case2() {
    ...
    // then compare the ingredient and final price against the expected
    // ingredient/price
    if (c2.get_ingredient(0).get_name() != "i1") {
      cout << "test 2 for rice ingredient 1 name failed, result unexpected"
           << endl;
    }
    if (c2.get_ingredient(0).get_price() != 0.2) {
      cout << "test 2 for rice ingredient 1 price failed, result unexpected"
           << endl;
    }
    if (c2.calculate_item_price() != (5 * 1.25 + 0.2)) {
      cout << "test 2 for chips price calculation failed, result unexpected"
           << endl;
    }
  }

  // Test case 3 for chips, with multiple ingredients, and setting values for
  // variables
  void test_chips_case3() {
    ...
    if (c3.get_ingredient(0).get_name() != "i1") {
      cout << "test 3 for rice ingredient 1 name failed, result unexpected"
           << endl;
    }
    if (c3.get_ingredient(0).get_price() != 0.2) {
      cout << "test 3 for rice ingredient 1 price failed, result unexpected"
           << endl;
    }
    if (c3.get_ingredient(1).get_name() != "ingredient3") {
      cout << "test 3 for rice ingredient 2 name failed, result unexpected"
           << endl;
    }
    if (c3.get_ingredient(1).get_price() != 0.8) {
      cout << "test 3 for rice ingredient 2 price failed, result unexpected"
           << endl;
    }
    if (c3.calculate_item_price() != (6.5 * 1.5 + 0.8 + 0.2)) {
      cout << "test 3 for chips price calculation failed, result unexpected"
           << endl;
    }
  }
