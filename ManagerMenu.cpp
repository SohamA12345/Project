...
void ManagerMenu::run(int& state_portal) {
  // create ManagerMenu, of class menu, which has tools 'create menu items',
  // 'show customer username + password', 'delete all customer details', 'log
  // out'. Each tool/method has their own description
  menu ManagerMenu;
  ...
  ManagerMenu.add(
      "Create menu items", 1,
      "Use this to create a new dish/drink/ingredient in the menu.");
  ManagerMenu.add("Show customer's username and password", 2,
                  "Helps if a customer forgot their username or password.");
  ManagerMenu.add(
      "Deletes all customer's username and login details", 3,
      "No way to recover customer's username or pass once removed.");
