...
void CustomerFoodDrink::run(int& state_customer_login) {
  ...
  CustomerFoodDrink.add("Food", 1,
                        "Wide range of cuisines categorised under Burger, "
                        "Chips, Noodles, Pasta, Pizza, Rice.");
  CustomerFoodDrink.add("Drink", 2, "Drinks Menu");
  CustomerFoodDrink.add(
      "Check-out", 3,
      "Cart has: " + cart_content + "Total Price: $" + to_string(total_price));
